...
package com.carloz.inputmethod.keyboard;

import static com.carloz.inputmethod.comm.Constants.ImeOption.FORCE_ASCII;
import static com.carloz.inputmethod.comm.Constants.ImeOption.NO_SETTINGS_KEY;
...
import com.carloz.inputmethod.comm.InputAttributes;
import com.carloz.inputmethod.compat.EditorInfoCompatUtils;
import com.carloz.inputmethod.compat.InputMethodSubtypeCompatUtils;
import com.carloz.inputmethod.flag.DebugFlags;
import com.carloz.inputmethod.keyboard.internal.KeyboardBuilder;
import com.carloz.inputmethod.keyboard.internal.KeyboardParams;
import com.carloz.inputmethod.keyboard.internal.KeysCache;
import com.carloz.inputmethod.utils.InputTypeUtils;
import com.carloz.inputmethod.utils.LogUtils;
import com.carloz.inputmethod.utils.LogUtils.Callback;
import com.carloz.inputmethod.utils.ScriptUtils;
import com.carloz.inputmethod.utils.SubtypeLocaleUtils;
import com.carloz.inputmethod.utils.XmlParseUtils;

/**
 * This class represents a set of keyboard layouts. Each of them represents a
 * different keyboard specific to a keyboard state, such as alphabet, symbols,
 * and so on. Layouts in the same {@link KeyboardLayoutSet} are related to each
 * other. A {@link KeyboardLayoutSet} needs to be created for each
 * {@link android.view.inputmethod.EditorInfo}.
 */
public final class KeyboardLayoutSet implements LogUtils.Callback {
	...
	// How many layouts we forcibly keep in cache. This only includes ALPHABET
	// (default) and
	// ALPHABET_AUTOMATIC_SHIFTED layouts - other layouts may stay in memory in
	// the map of
	// soft-references, but we forcibly cache this many alphabetic/auto-shifted
	// layouts.
	private static final int FORCIBLE_CACHE_SIZE = 4;
	// By construction of soft references, anything that is also referenced
	// somewhere else
	// will stay in the cache. So we forcibly keep some references in an array
	// to prevent
	...
	public static final class Params {
		...
		// Sparse array of KeyboardLayoutSet element parameters indexed by
		// element's id.
	...
	public Keyboard getKeyboard(final int baseKeyboardLayoutSetElementId) {
		...
		ElementParams elementParams = mParams.mKeyboardLayoutSetElementIdToParamsMap.get(keyboardLayoutSetElementId);
		if (elementParams == null) {
			elementParams = mParams.mKeyboardLayoutSetElementIdToParamsMap.get(KeyboardId.ELEMENT_ALPHABET);
		}
		// Note: The keyboard for each shift state, and mode are represented as
		// an elementName
		// attribute in a keyboard_layout_set XML file. Also each keyboard
		// layout XML resource is
		...
		try {
			...
			LogUtils.e(getLogUtilsCallback(), "Can't create keyboard: " + id + ", " + e);
	...
	private Keyboard getKeyboard(final ElementParams elementParams, final KeyboardId id) {
		...
		if (cachedKeyboard != null) {
				LogUtils.d(getLogUtilsCallback(), "keyboard cache size=" + sKeyboardCache.size() + ": HIT  id=" + id);
		...
		if ((id.mElementId == KeyboardId.ELEMENT_ALPHABET || id.mElementId == KeyboardId.ELEMENT_ALPHABET_AUTOMATIC_SHIFTED) && !mParams.mIsSpellChecker) {
			...
			sForcibleKeyboardCache[0] = keyboard;
				LogUtils.d(getLogUtilsCallback(), "forcing caching of keyboard with id=" + id);
		}
			LogUtils.d(getLogUtilsCallback(), "keyboard cache size=" + sKeyboardCache.size() + ": " + ((ref == null) ? "LOAD" : "GCed") + " id=" + id);
		return keyboard;
	}
	...
	public static final class Builder {
		...
		public Builder(final Context context, final EditorInfo ei) {
			...
			params.mNoSettingsKey = InputAttributes.inPrivateImeOptions(mPackageName, NO_SETTINGS_KEY, editorInfo);
		}
		...
		public Builder setSubtype(final InputMethodSubtype subtype) {
			...
			final boolean deprecatedForceAscii = InputAttributes.inPrivateImeOptions(mPackageName, FORCE_ASCII, mParams.mEditorInfo);
			final boolean forceAscii = EditorInfoCompatUtils.hasFlagForceAscii(mParams.mEditorInfo.imeOptions) || deprecatedForceAscii;
			final InputMethodSubtype keyboardSubtype = subtype;
			mParams.mSubtype = keyboardSubtype;
			mParams.mKeyboardLayoutSetName = KEYBOARD_LAYOUT_SET_RESOURCE_PREFIX + SubtypeLocaleUtils.getKeyboardLayoutSetName(keyboardSubtype);
			return this;
		}
		...
		public KeyboardLayoutSet build() {
			...
			final String packageName = mResources.getResourcePackageName(R.xml.keyboard_layout_set_qwerty);
		...
		private void parseKeyboardLayoutSet(final Resources res, final int resId) throws XmlPullParserException, IOException {
		...
		private void parseKeyboardLayoutSetContent(final XmlPullParser parser) throws XmlPullParserException, IOException {
			while (parser.getEventType() != XmlPullParser.END_DOCUMENT) {
				final int event = parser.next();
				if (event == XmlPullParser.START_TAG) {
					final String tag = parser.getName();
		...
		private void parseKeyboardLayoutSetElement(final XmlPullParser parser) throws XmlPullParserException, IOException {
			final TypedArray a = mResources.obtainAttributes(Xml.asAttributeSet(parser), R.styleable.KeyboardLayoutSet_Element);
			try {
				...
				final int elementName = a.getInt(R.styleable.KeyboardLayoutSet_Element_elementName, 0);
				elementParams.mKeyboardXmlId = a.getResourceId(R.styleable.KeyboardLayoutSet_Element_elementKeyboard, 0);
				elementParams.mProximityCharsCorrectionEnabled = a.getBoolean(R.styleable.KeyboardLayoutSet_Element_enableProximityCharsCorrection, false);
		...
		private void parseKeyboardLayoutSetFeature(final XmlPullParser parser) throws XmlPullParserException, IOException {
			final TypedArray a = mResources.obtainAttributes(Xml.asAttributeSet(parser), R.styleable.KeyboardLayoutSet_Feature);
			try {
				final int scriptId = a.getInt(R.styleable.KeyboardLayoutSet_Feature_supportedScript, ScriptUtils.SCRIPT_LATIN);
	...
	public boolean getDebug() {
		// TODO Auto-generated method stub
		return DebugFlags.KeyboardLayoutSet;
	}
}
